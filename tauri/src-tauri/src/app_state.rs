...
pub struct AppState {
    ...
    /// The Privacy Consent Store's camera-in-use flag for Teams (see `registry_monitor`).
    /// Reliable for a physical webcam, but a virtual-camera passthrough (OBS, NVIDIA
    /// Broadcast) can leave it never updating, since that path does not always route
    /// through the Frame Server capability check the store is fed by. Kept as
    /// `recompute_video`'s fallback for when there is no meeting window to read a camera
    /// button from yet.
